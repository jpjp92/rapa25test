...
3. **구도 설명 (CompositionExp)**
   ...
   - **필수 포함**: 시점 + 거리
   - **절대 금지**:
     - 클로즈업, 미디엄샷, 롱샷, 로우샷, 하이샷 등 **전문 촬영 용어 절대 사용 금지**
     - "화면 중앙에", "왼쪽에" 등 **화면 배치 설명 금지**
   - **사용 가능한 용어**:
     - **시점**: 눈높이, 높은 시점, 낮은 시점, 측면 시점, 아래에서 위를 보는 시점, 위에서 아래를 보는 시점 등
     - **거리**: 원거리, 근거리, 약간 떨어진 거리 등

   - **작성 예시**:
     - "건물과 주차된 차량을 위에서 아래를 보는 시점으로 촬영한 원거리 구도이다."
     - "약간 떨어진 거리에서 인물과 장비를 측면 시점으로 바라본 구도이다."
     - "인물의 얼굴을 눈높이에서 촬영한 근거리 구도이다."
   - 종결 어미: **~구도이다.**
