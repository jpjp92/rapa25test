...
**최우선 규칙 (다른 어떤 규칙보다 가장 우선함)**
- EraCategory(시대 구분)는 아래 기준으로만 판정하며,
- 장면/색감/구도/객체 설명 규칙은 시대 구분에 절대 영향을 주지 않는다.
...
#### 최우선 규칙 (모든 설명문에 적용):
**위치 정보 표현 제한 - 절대 준수!**
- **금지**: 앞, 뒤, 왼쪽, 오른쪽 구체적인 방향/위치 지시어 사용 금지
- **허용**: "옆", "근처", "주변" 등 상대적인 위치 표현만 사용
- 잘못된 예: "인물 오른쪽에는", "화면 왼쪽 상단에"
- 올바른 예: "인물 옆에는", "배경에는"
...
1. **장면 설명 (SceneExp)**
   ...
   - **제외 요소**: 색감(ColortoneExp), 구도(CompositionExp), 객체1,2, 설명(ObjectExp), 실내/실외/야외 같은 공간 ‘분류’ 단어
   - ** 절대금지 요소**: SceneExp에서는 색에 대한 어떤 표현도 포함하지 않는다.
         색상명(노란, 파란, 붉은, 어두운, 밝은, 선명한 등),
         색조 표현(톤, 명암, 대비, 따뜻한 분위기, 차분한 분위기 등),
         색을 암시하는 표현(밝게, 어둡게, 환하게 등).
   - **작성 방식**: 구체적인 장소(방, 주방, 거실, 거리, 공원, 창가, 책상 앞 등), 환경, 행동 중심으로 묘사 (추상적 분위기 표현 최소화)
   - **중복 주의**: 색감 설명과 동일한 수식어 사용 금지 (차분한, 밝은, 어두운, 활기찬, 따뜻한 등)
   - 종결 어미: **~장면이다.**

2. **색감 설명 (ColortoneExp)**
   - **목적**: 이미지의 색채적 특성만 전달
   - **포함 요소**: 주요 색상, 명암, 톤, 채도, 장면 기반 색감
   - **작성 방식**: "색상 + 대상"을 2-3개 조합하여 전체 색감 묘사
     - 예: "옅은 베이지색 벽", "푸른빛이 도는 하늘", "어두운 의상"
   - **제외 요소**: 객체의 형태나 질감, 장면의 분위기, 카메라 구도
   - ** 절대금지 요소**: 차분한 분위기, 차분한 색감 
   - **중요**: EraCategory가 전통(1)인 경우, 위 "한국전통색상 사용 규칙"을 참고하여 전통색상명 사용
   - 종결 어미: **~색감이다.**

3. **구도 설명 (CompositionExp)**
   - **목적**: 카메라 시점과 거리감만 전달 (화면 배치 제외)
   - **필수 포함**: 시점 + 거리
   - **절대 금지**:
     - 클로즈업, 미디엄, 롱, 로우, 하이 등 **전문 촬영 용어 절대 사용 금지**
     - *영어 표현 사용 금지 -> 한국어 표현으로 대체*
     - "화면 중앙에", "왼쪽에" 등 **화면 배치 설명 금지**
   - **제외 요소**: 색상, 객체의 세부 묘사, 장면의 내용 (배경에 무엇이 있다는 등), 색감(ColortoneExp)
   - **거리 분류 방법**:
     - 이미지 속 ‘주된 피사체’(인물·사물·건물·무대·풍경 등)가 화면에서 차지하는 크기를 기준으로 거리감을 단순하게 분류
     - 단, 설명문에는 ‘근접·가까운·중간·원거리’ 같은 거리명을 직접 쓰지 말고, 해당 거리감을 자연스럽게 변주한 한국어 표현으로 서술
   - **거리 판단 기준**:
     - 피사체가 화면을 크게 채우고 디테일이 강조되면 근접
     - 피사체가 화면 절반 정도 크기이며 일부 주변 요소가 보이면 가까운
     - 피사체와 주변 환경이 균형 있게 보이며 상반신·객체 전체가 절반 이하 크기면 중간
     - 피사체가 작게 보이거나 공간 전체가 중심이면 원거리
   - **거리 표현 규칙**:
     - 거리명 대신 상황을 설명하는 말로 다양하게 표현할 것(예: 아주 가까운 위치에서 / 디테일을 강조하며 / 비교적 떨어져서 / 적당한 간격을 두고 / 공간 전체가 드러나도록 멀리서 / 전체를 조망하듯 등)
     - 특정 표현을 반복하지 말고 문맥에 맞게 변형하여 사용할 것
     - 피사체 종류(인물·사물·무대·풍경)에 관계없이 적용되는 거리감 묘사를 생성할 것
   - **작성 방식**:
     - "~을 ~시점으로 촬영한 ~구도이다."
     - "~을 ~시점에서 촬영한 ~구도이다."
   - 종결 어미: **~구도이다.**
...
4. **객체1 설명 (ObjectExp1)**
   ...
   - **절대 금지**: 색상 표현 (빨간, 파란, 노란, 흰, 검은, 회색, 베이지, 밝은, 어두운 등), 색감(ColortoneExp), 왼쪽/오른쪽 등 방향에 대한 표현 금지
...
4.  Explanation이 5개 문장을 순서대로 합친 것인가?
   - SceneExp + ColortoneExp + CompositionExp + ObjectExp1 + ObjectExp2
...
{{
  ...
  "annotation_info": {{
    "SceneExp": "[실제 장면 설명으로 교체 - 장소, 환경, 분위기 (색상 제외)]",
    "ColortoneExp": "[실제 색감 설명으로 교체 - 색상, 명암, 톤]",
    "CompositionExp": "[실제 구도 설명으로 교체 - 카메라 각도, 원근감 (색상 제외)]",
