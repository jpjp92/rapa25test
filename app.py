...
아래 조건 중 **하나라도 해당하면** LocationCategory는 **무조건 기타(4)**로 분류하며, **실내 / 실외 / 혼합 판정은 시도하지 않는다.**

**기타(4) 분류 기준**
(A) 비현실적 또는 합성·비실사 이미지
- 일러스트, 애니메이션, 카툰 스타일
- CG/3D 모델·렌더링

(B) 시각적 오류·촬영 문제로 장소 판정 불가
- 강한 모션블러(화면 흔들림)
- 화면 전환/겹침(디졸브, 페이드, 멀티 노출 등)
- 빛 번짐·플레어·전면 과노출
- 암부 노이즈로 인해 객체·배경 식별 불가
- 개인정보 보호 블러(얼굴/배경 흐림 포함)
- 초점 완전 이탈로 구조물 판별 불가

(C) 장소 판정을 방해하는 멀티 화면 구성
...
(D) 객체·장소가 명확히 보이지 않는 경우
- 카메라 흔들림으로 인해 피사체의 외곽선이 끊기거나 번져 보임
- 배경의 선·면·형태가 흐트러져 공간의 형태를 정확히 판단할 수 없음
- 인위적 흐림(보정 Blur, 아웃포커싱, 보정 효과)로 보이는 경우
- 모션블러 영향으로 사람·사물의 위치나 자세가 명확하지 않음
- 이미지의 **대부분(50% 이상)**이 흐림 처리되어 피사체의 형태가 유지되지 않음

2. **EraCategory (시대 구분) - 반드시 아래 순서대로 판정**
   **판정 순서:**
   1. 먼저 이미지의 **모든 요소**(인물, 건축물, 소품)를 확인
   2. 전통 요소와 현대 요소가 **모두** 있으면 → 혼합(3)
   3. 전통 요소만 있으면 → 전통(1)
   4. 현대 요소만 있으면 → 현대(2)

   **분류 기준:**
   - **전통(1)**: **전통 요소만** 존재하는 경우
     - 전통 건축물(한옥, 성곽, 사찰), 전통 복장(한복, 갑옷), 전통 소품만 있는 경우

   - **현대(2)**: **현대 요소만** 존재하는 경우
     - 현대 건축물, 현대 복장(일상복, 정장), 현대 소품만 있는 경우

   - **혼합(3)**: **전통 요소와 현대 요소가 한 화면에 공존**
     - 전통 건축물 + 현대 소품,의상,물건
     - 전통 복장 + 현대 복장 
     - 현대 배경 + 전통 소품
   ...
   **중요 원칙:**
   - 화면에 보이는 **모든 현대 요소**는 판정에 반영
   - 크기가 작거나 배경에 있어도 **명확히 보이면** 판정 기준에 포함
