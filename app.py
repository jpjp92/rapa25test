...
**최우선 규칙**
- LocationCategory(장소 구분)는 아래 기준으로만 판정하며,
...
#### 최우선 규칙 (모든 설명문에 적용):
**사람 수 표현 - 절대 준수!**
- 1-2명: **SceneExp에서 처음 언급할 때만** 정확한 수 명시 (한 명, 두 명). 이후 다른 설명문에서는 '해당 인물', '그' 등으로 지칭하거나 주어 생략.
...
3. **구도 설명 (CompositionExp)**
   ...
   - **절대 금지**: "한 명의", "두 명의", "여러 명의" 등 **사람 수 표현을 절대 사용하지 않는다**.
   - **권장 시작**: **"화면 중앙에"**, **"낮은 시점에서"**, **"눈높이에서"** 등 위치/시점을 주어로 시작. 인물을 언급할 때는 수량 없이 **"인물이"**, **"인물의"**만 사용.
   - 종결 어미: **~구도이다.**
...
4. **객체1 설명 (ObjectExp1)**
   ...
   - **절대 금지**: 색상 표현 (빨간, 파란, 노란, 흰, 검은, 회색, 베이지, 밝은, 어두운 등)
   - **제외 요소**: 전체적 분위기(SceneExp), 카메라 구도, 색감(ColortoneExp)
   ...
   - **절대 금지**: 문장을 **"한 명의 인물이"로 시작하지 않는다**.
   - **권장 시작**: **"해당 인물은"**, **"그는"**, **"그녀는"** 또는 주어 생략하고 바로 묘사 시작.
...
5. **객체2 설명 (ObjectExp2)**
   ...
   - **포함 요소**: 형태, 크기, 질감, 패턴, 자세, 동작, 의상/구조
   - **절대 금지**: 색상 표현 (빨간, 파란, 노란, 흰, 검은, 회색, 베이지, 밝은, 어두운 등)
   - **제외 요소**: 전체적 분위기(SceneExp), 카메라 구도, 객체1과 중복되는 객체, 색감(ColortoneExp)
   - **동작 묘사 원칙**: 이미지에서 명확히 관찰되는 자세/동작만 기술 (추측 금지)
     - 허용: "서 있다", "앉아 있다", "손을 뻗고 있다", "고개를 돌리고 있다"
     - 금지: "작업하고 있다" (모호함), "물건을 들고 있다" (불명확)
   - **사람 수 표현: 상단 "최우선 규칙" 참조 (3명 이상은 무조건 "여러 명")**
   - **주의 사항**:
     - SceneExp에서 이미 언급된 대상을 다시 묘사할 때는 "한 명의 인물이"를 반복하지 말고, **"해당 인물은"**으로 지칭하거나 문맥상 명확하면 주어를 생략한다.
     - 객체1과 다른 대상을 묘사할 때는 **"다른 한 남자는"**, **"또 다른 인물은"** 처럼 **'은/는' 조사**를 사용하여 자연스럽게 연결한다.
   - 객체가 사물인 경우 표면적 느낌도 같이 설명
   - 종결 어미: **~하고 있다.** 또는 **~이 있다.**
