...
2. **EraCategory (시대 구분)**
   - 현대(2): 현대 복장을 입은 모든 경우 (일상복, 정장, 캐주얼 등)
   - 전통(1): 사극, 시대극, 재연극 등 이전 역사적 시대 상황에 놓여 있는 이미지 (한복, 갑옷, 전통 의상 착용)
   - 혼합(3): 시대극에 엉뚱하게 현대의 사람이나 물건이 나와서 혼재된 경우
   - 기타(4): 그 외에 분류하기 어려운 경우
...
1. **장면 설명 (SceneExp)**
   - **목적**: 이미지의 전체적인 맥락과 상황 전달
   - **포함 요소**: 장소의 종류, 전체적인 환경, 사람들의 행동/상황
   - **제외 요소**: 색상(ColortoneExp), 카메라 각도(CompositionExp), 개별 객체 세부사항(ObjectExp)
   - **작성 방식**: 구체적인 장소와 상황을 중심으로 묘사하고, 필요시 분위기 표현 추가
   - **중복 주의**: 색감 설명과 동일한 수식어(차분한, 밝은, 어두운 등) 사용 금지
   - 종결 어미: **~장면이다.**
...
3. **구도 설명 (CompositionExp)**
   ...
   - **포함 요소**: 카메라 높이/각도, 원근감, 화면 내 배치, 프레이밍
   - **제외 요소**: 색상, 객체의 세부 묘사, 장면의 내용
   - 종결 어미: **~구도이다.**
   - 예시: "높은 시점에서 내려다보는 항공 구도이다."

4. **객체1 설명 (ObjectExp1)**
   - **목적**: 이미지의 주요 객체 하나에 대한 물리적 특징 전달
   - **포함 요소**: 형태, 크기, 질감, 패턴, 동작, 자세, 의상/구조
   - **절대 금지**: 색상 표현 (빨간, 파란, 노란, 흰, 검은, 회색, 베이지 등 모든 색상 단어)
