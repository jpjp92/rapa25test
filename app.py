...
1. **장면 설명 (SceneExp)**
   ...
   - ** 절대금지 요소**: SceneExp에서는 색에 대한 어떤 표현도 포함하지 않는다.
         ...
         **시점, 피사체**가 담긴 범위 표현 (자연, 풍경 중심 이미지의 경우) 
...
3. **구도 설명 (CompositionExp)**
   - **목적**: 카메라의 시점(높이·위치)과 담긴 범위(전신·상반신·넓게 등)만 간단하게 전달
   - **필수 포함**: 시점 + 피사체가 담긴 범위 
   - **시점**: 눈높이, 낮은 위치, 높은 위치, 멀리서, 가까운 위치, ~을 내려다보는 위치 등
   - **피사체가 담긴 범위**: 인물의 전신을, 인물의 상반신을, 인물의 하반신을, 여러 인물을, 건물 전체를, ~전경을 넓게, 주변 공간을 포함해, 공간 전체를 등
   - **절대 금지**:
     - 클로즈업, 미디엄, 롱, 로우, 하이 등 **전문 촬영 용어 절대 사용 금지**
     - *영어 표현 사용 금지 -> 한국어 표현으로 대체*
     - "화면 중앙에", "왼쪽에" 등 **화면 배치 설명 금지**
     - **자연, 풍경, 경관 중심 이미지**는 **범위 표현 금지**
